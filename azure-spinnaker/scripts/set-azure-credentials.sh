...
my_app_name="spinnakertrialapp2"
...
sed -i -u s/MY_AZURE_TENANT_ID/$my_tenant_id/g $HOME/spinnaker-local.yml
echo "sed -i -u s/MY_AZURE_CLIENT_ID/$my_client_id/g $HOME/spinnaker-local.yml"
sed -i -u s/MY_AZURE_CLIENT_ID/$my_client_id/g $HOME/spinnaker-local.yml
echo "sed -i -u s/MY_AZURE_APP_KEY/$my_app_key/g $HOME/spinnaker-local.yml"
sed -i -u s/MY_AZURE_APP_KEY/$my_app_key/g $HOME/spinnaker-local.yml
# TODO: Enable Key vault support 
echo "sed -i -u s/MY_AZURE_RESOURCE_GROUP/$my_key_vault_resource_group/g $HOME/spinnaker-local.yml"
sed -i -u s/MY_AZURE_RESOURCE_GROUP/$my_key_vault_resource_group/g $HOME/spinnaker-local.yml
echo "sed -i -u s/MY_AZURE_KEY_VAULT/$my_key_vault_name/g $HOME/spinnaker-local.yml"
sed -i -u s/MY_AZURE_KEY_VAULT/$my_key_vault_name/g $HOME/spinnaker-local.yml
echo "sed -i -u s/MY_AZURE_PACKER_RESOURCE_GROUP/$my_packer_resource_group/g $HOME/spinnaker-local.yml"
sed -i -u s/MY_AZURE_PACKER_RESOURCE_GROUP/$my_packer_resource_group/g $HOME/spinnaker-local.yml
echo "sed -i -u s/MY_AZURE_PACKER_STORAGE_ACCOUNT/$my_packer_storage_account/g $HOME/spinnaker-local.yml"
sed -i -u s/MY_AZURE_PACKER_STORAGE_ACCOUNT/$my_packer_storage_account/g $HOME/spinnaker-local.yml
...
ROSCO_CONFIG_SOURCE="https://raw.githubusercontent.com/scotmoor/azure-quickstart-templates/master/azure-spinnaker/scripts/rosco-local.yml"
sudo curl -o "$my_azure_spinnaker_config_path/rosco-local.yml" $ROSCO_CONFIG_SOURCE
...
sudo curl -X POST --header "Content-Type: application/json" --header "Accept: */*" -d "[ {\"upsertLoadBalancer\": {\"cloudProvider\": \"azure\",\"appName\": \"$APP_NAME\",\"loadBalancerName\": \"$APP_GATEWAY_NAME\",\"stack\": \"$STACK\",\"detail\": \"$APPG_DETAIL\",\"credentials\": \"my-azure-account\",\"region\": \"$default_location\",\"probes\": [{\"probeName\": \"healthcheck1\",\"probeProtocol\": \"HTTP\",\"probePort\": \"localhost\",\"probePath\": \"/\",\"probeInterval\": 30,\"unhealthyThreshold\": 8,\"timeout\": 120}],\"loadBalancingRules\": [{\"ruleName\": \"$APP_GATEWAY_NAME-rule0\",\"protocol\": \"HTTP\",\"externalPort\": 80,\"backendPort\": 80,\"probeName\": \"$APP_GATEWAY_NAME-probe\",\"persistence\": \"None\", \"idleTimeout\": 4 }], \"name\":\"$APP_GATEWAY_NAME\",\"user\": \"[anonymous]\"}}]" "http://localhost:7002/azure/ops"
echo "Demo Load Balancer $APP_GATEWAY_NAME created"

# Create a pipeline with bake and deploy stage
echo "Creating default piple \"$PIPELINE_NAME\" in Spinnaker...."
CREATE_PIPELINE_DATA="{\"application\":\"$APP_NAME\",\"appConfig\":{},\"keepWaitingPipelines\":false,\"limitConcurrent\":true,\"parallel\":true,\"name\":\"$PIPELINE_NAME\",\"stages\":[{\"baseLabel\":\"release\",\"baseName\":\"test\",\"baseOs\":\"ubuntu\",\"cloudProviderType\":\"azure\",\"extendedAttributes\":{},\"name\":\"Bake\",\"osType\":\"Linux\",\"package\":\"gedit\",\"refId\":\"1\",\"regions\":[\"$default_location\"],\"requisiteStageRefIds\":[],\"type\":\"bake\",\"user\":\"[anonymous]\"},{\"clusters\":[{\"account\":\"my-azure-account\",\"application\":\"$APP_NAME\",\"capacity\":{\"max\":1,\"min\":1,\"useSourceCapacity\":false},\"cloudProvider\":\"azure\",\"detail\":\"$PIPELINE_DETAIL\",\"freeFormDetails\":\"$PIPELINE_DETAIL\",\"image\":{\"imageName\":\"\",\"isCustom\":\"true\",\"offer\":\"\",\"ostype\":\"\",\"publisher\":\"\",\"region\":\"$default_location\",\"sku\":\"\",\"uri\":\"\",\"version\":\"\"},\"interestingHealthProviderNames\":[],\"loadBalancerName\":\"$APP_GATEWAY_NAME\",\"name\":\"$APP_NAME-$STACK-$PIPELINE_DETAIL\",\"osConfig\":{\"adminPassword\":\"!Qnti**234\",\"adminUserName\":\"spinnakeruser\"},\"region\":\"$default_location\",\"selectedProvider\":\"azure\",\"sku\":{\"capacity\":1,\"name\":\"Standard_DS1_v2\",\"tier\":\"Standard\"},\"stack\":\"$STACK\",\"type\":\"createServerGroup\",\"upgradePolicy\":\"Manual\",\"user\":\"[anonymous]\",\"viewState\":{\"allImageSelection\":null,\"disableImageSelection\":true,\"disableStrategySelection\":false,\"hideClusterNamePreview\":false,\"imageId\":null,\"instanceProfile\":\"custom\",\"loadBalancersConfigured\":true,\"mode\":\"createPipeline\",\"readOnlyFields\":{},\"securityGroupsConfigured\":true,\"submitButtonLabel\":\"Add\",\"templatingEnabled\":true,\"useAllImageSelection\":false,\"usePreferredZones\":true,\"useSimpleCapacity\":true}}],\"name\":\"Deploy\",\"refId\":\"2\",\"requisiteStageRefIds\":[\"1\"],\"type\":\"deploy\"}],\"triggers\":[]}"
